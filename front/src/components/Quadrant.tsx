import { Chain, ethos } from "ethos-connect";
import { memo, useEffect, useRef } from "react";
import { BOARD, PACKAGE_ID } from "../deployed_addresses.json";
...
const Quadrant = memo(
    ...
    useEffect(() => {
      if (ref.current) {
        ...
        const get_quadrant = (n_pixel: number) => {
          ...
          return quadrants[x ? (y ? 3 : 1) : y ? 2 : 0];
        };
        ...
        for (let i = 0; i < img_bytes.length; i += 4) {
          ...
          const x_offset =
            n_pixel % 400 >= 200 ? (n_pixel % 400) - 200 : n_pixel % 400;
        ...
        createImageBitmap(img_data).then((bitmap) =>
      ...
      <canvas
        onClick={(e) => {
          ...
          if (rect) {
            // && (e.ctrlKey || e.metaKey)
            const x = Math.floor(
              ((e.clientX - rect.left) / (rect.right - rect.left)) * 400
            );
            const y = Math.floor(
              ((e.clientY - rect.top) / (rect.bottom - rect.top)) * 400
            );
            if (!wallet) {
              return ethos.showSignInModal();
            }
  ...
  (prev, next) =>
    JSON.stringify([prev.quadrants.map(([, digest]) => digest), prev.color]) ===
    JSON.stringify([next.quadrants.map(([, digest]) => digest), next.color])
);
